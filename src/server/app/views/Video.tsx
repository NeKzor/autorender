...
const formatRenderTime = (data: Data) => {
  ...
  return data.render_time < 60
    ? `${data.render_time} seconds`
    : `${(data.render_time / 60).toFixed(2)} minutes`;
};
    ...
    <>
      ...
      {data && (
        <>
          ...
          {isQueueRoute ? (
            ...
            <>
              ...
              {!data.video_url &&
                data.pending !== PendingStatus.FinishedRender && (
                  <div>
                    Video is currently queued for rendering. Please come back
          ...
          <div>
            Render node:{" "}
            {data.rendered_by_username !== null ? (
          ...
          {data.demo_time_score !== null && (
            <div>Time Score: {formatCmTime(data.demo_time_score)}</div>
          )}
          {data.demo_portal_score !== null && (
            <div>Portal Score: {data.demo_portal_score}</div>
          )}
          ...
          {metadata.timestamp !== null && (
            <div>Timestamp: {formatTimestamp(metadata.timestamp)}</div>
          )}
            ...
            <>
              ...
              <ul>
                {metadata.segments!.map((segment, index, segments) => {
                  ...
                  return (
                    <li>
                      {formatToSeconds(segment.ticks, data)} ({segment.ticks}) -{" "}
                      {formatToSeconds(totalTicks, data)} ({totalTicks}) -{" "}
                      {segment.name}
                    </li>
                  );
                })}
