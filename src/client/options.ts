...
import { downloadSourceAutoRecord, gameModsWhichSupportWorkshop, getConfig, getConfigOnly } from "./config.ts";
...
const runCheck = async (options: Options) => {
  ...
  try {
    ...
    if (autorender && autorender.tag_name !== autorenderVersion) {
      console.log(
        colors.green(
          `A new release of autorender is available: ${
            colors.cyan(autorenderVersion)
          } ${colors.white("→")} ${colors.cyan(autorender.tag_name)}`,
        ),
      );
      console.log(
        colors.italic.gray(
    ...
    if (sar.tag_name !== config.sar.version) {
      console.log(
        colors.green(
          `A new release of SourceAutoRecord is available: ${
            colors.cyan(config.sar.version)
          } ${colors.white("→")} ${colors.cyan(sar.tag_name)}`,
        ),
      );
      console.log(
        colors.italic.gray(
